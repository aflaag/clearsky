...
Model merging -- combining independently fine-tuned checkpoints directly in weight space -- has recently been shown to recover much of the performance of joint multi-task training for classification and language models \cite{ilharco2023editing,yadav2023ties} with little to no additional training. However, we find that applying these methods zero-shot to conditional diffusion models solving distinct low-level image restoration tasks fails to meet basic performance expectations. To overcome this limitation, we extend the standard merging paradigm by introducing a mandatory, lightweight post-merge fine-tuning phase for all evaluated methods, which successfully resolves interference in the merged weight space.

Concretely, we (i) pretrain a single shared initialization $\theta_0$ on a generic conditional-restoration pretext task built from all our data, (ii) independently fine-tune three specialist DDPMs from $\theta_0$, one per degradation, (iii) merge the resulting task vectors with Task Arithmetic \cite{ilharco2023editing}, TIES-Merging \cite{yadav2023ties}, and DARE-TIES \cite{yu2024dare}, and (iv) subject all merged models to a brief post-merge fine-tuning step to recover target performance, optimizing coefficients via a two-stage grid search.
...
Given task vectors $\tau_t = \theta_t - \theta_0$ for $t \in \{\text{SR}, \text{IR}, \text{SU}\}$, we first construct combined weight configurations using:
\begin{itemize}
...
\paragraph*{Post-merge fine-tuning.} In early experiments, we noticed that zero-shot application of Task Arithmetic, TIES, and DARE-TIES failed to meet baseline performance expectations, resulting in significant image artifacts and a severe drop in reconstruction fidelity due to destructive weight interference. To address this, we introduce a subsequent, mandatory fine-tuning stage for all three merged checkpoints. This phase consists of a brief training run utilizing a low learning rate and an aggregated dataset containing balanced mixtures of all three tasks, smoothly restoring cross-task synergy and structural regularities.
...
\paragraph*{Hardware / compute.} \todo{GPU type/count, total training, post-merge fine-tuning, and grid-search compute budget.}

% ------------------------------------------------------------------------------
\section{Results}
\label{sec:results}

\subsection{Task Arithmetic \todo{"error tracking pipeline"?}}

This section contains all the experiments performed initially on task arithmetic, and subsequently repliacted on the other methods directly on the last iteration in other to maximize the results. \todo{write better}
...
\paragraph*{Findings} The whole arc reduces to one lesson: \textbf{the choice of evaluation metric
...
\begin{table}[ht]
	...
	\begin{tabular}{p{3.2cm}p{5.2cm}p{4.1cm}}
		\toprule
		\textbf{Experiment}                                                                          & \textbf{Main objective} & \textbf{Outcome} \\
		\midrule
		Baseline merge                                                                               &
		Standard TIES merging with default aggregation.                                              &
		Merged model consistently underperformed both specialists and, in several tasks, the pretrained model.                                    \\

		Diagnostic analysis                                                                          &
		Determine whether the performance loss originated from the specialists or the merge process. &
		Specialists were validated; the merge procedure was identified as the primary bottleneck.                                                 \\

		Improved merging                                                                             &
		Evaluate alternative aggregation strategies and expanded hyperparameter search.              &
		Clear improvement over the baseline merge, with better task retention and higher restoration quality.                                     \\

		Extended evaluation                                                                          &
		Assess merges on unseen combinations of degradations using a two-stage selection procedure.  &
		Evaluation framework completed, but final testing was interrupted by a dataset configuration error.                                       \\
		\bottomrule
	\end{tabular}
	\caption{Evolution of the TIES-merging experiments and their main outcomes.}
	\label{tab:ties_evolution}
\end{table}
